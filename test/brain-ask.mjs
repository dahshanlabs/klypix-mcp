...
// ── correction-aware: a stale hit ALWAYS carries its live correction ─────────
{
    ...
    const current = struct.cards.find(c => /now WIRED/.test(c.text || ''));
    struct.connections.push({ fromId: stale.id, toId: current.id, label: 'superseded by' });
...
// Semantic fusion must depend on rank order, not one model family's raw cosine
// distribution. This protects future embedding upgrades from silently changing
// answers merely because their scores cluster in a different numeric range.
{
    const struct = {
        cards: [
            { id: 'a', type: 'text', text: 'Relay: queued envelopes resume delivery after disconnected operation.', title: 'a', tags: [], area: 'Relay', createdAt: 1 },
            { id: 'b', type: 'text', text: 'Canvas: selection handles use a translucent outline.', title: 'b', tags: [], area: 'Canvas', createdAt: 2 },
            { id: 'c', type: 'text', text: 'Billing: annual invoices are generated by the provider.', title: 'c', tags: [], area: 'Billing', createdAt: 3 },
        ],
        connections: [],
    };
    const spread = rankForQuestion(struct, 'How does work continue without a connection?', {
        semantic: new Map([['a', 0.91], ['b', 0.31], ['c', -0.12]]),
        k: 3,
    });
    const clustered = rankForQuestion(struct, 'How does work continue without a connection?', {
        semantic: new Map([['a', 0.991], ['b', 0.987], ['c', 0.982]]),
        k: 3,
    });
    ok(
        spread.hits.map(h => h.card.id).join('') === clustered.hits.map(h => h.card.id).join('')
        && spread.hits[0]?.card.id === 'a',
        'ask: semantic ordering is invariant to model score distribution',
    );

    const anchored = rankForQuestion({
        cards: [
            { id: 'near', type: 'text', text: 'Release: a deployment finished and health checks passed.', title: 'near', tags: [], area: 'Release', createdAt: 1 },
            { id: 'exact', type: 'text', text: 'Release: PR #440 merged successfully into main.', title: 'exact', tags: [], area: 'Release', createdAt: 2 },
        ],
        connections: [],
    }, 'Did PR #440 merge?', {
    ...
    ok(anchored.hits[0]?.card.id === 'exact', 'ask: exact PR/path/version anchors can override a near semantic tie');
}

...
server.registerTool('brain_garden', {
  ...
  description: 'Tidy an over-grown brain WITHOUT losing anything. Two phases: call it with no apply to get the areas that have accumulated more old cards than anyone re-reads (deterministic: >3 cards older than 14 days beyond an area\'s newest 8; Focus/Instructions/Archive/Open-questions protected) plus their card text; YOU write one tight synthesis per area; then call again with apply:true and syntheses:[{title, synthesis}]. Each area gets a 🌿 synthesis card, the originals are stamped "⤵ consolidated", moved to Archive, and arrowed to the synthesis — nothing is deleted, and one undo un-gardens. Run it when brain_insights or the brief shows an area has grown noisy.',
  inputSchema: {
    canvas: z.string().optional().describe('Brain canvas filename/path. Defaults to the project brain ("brain").'),
